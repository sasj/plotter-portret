...
*wat is het verschil met `circle()`?*  
Bij `circle()` is de `0,0` positie in het midden, bij `rect()` is dat standaard (default) links bovenin.
...
function draw() {
  ...
  circle(200,100,afmetingRondjes);
  circle(200,200,afmetingRondjes);
  circle(200,300,afmetingRondjes);
